...
# When given a positive 'amt', a single rotation will rotate right, causing the first element to move to the last index
# When given a negative 'amt', a single rotation will rotate left, causing the last element to move to the first index
...
def my_rotate!(array, amt)
    if amt > 0
        amt.times do 
            # shift gets first number and shovel it to the end
            ele = array.shift
            array << ele
        end
    else
